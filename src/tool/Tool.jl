   ...
   for (schema, schemadef) in db_analysis
      ...
      for (table,tabledef) in schemadef
         ...
         for pkcol in tabledef[:pk]
            ...
            # Skip if we already handled this column
            if pkcol in referencing_cols continue end
...
function generate_structs_from_object_model(object_model::Dict, outdir::String)
